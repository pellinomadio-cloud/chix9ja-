...
const LiveNotifications: React.FC<LiveNotificationsProps> = ({ preferences }) => {
  ...
  return (
    <div className={`fixed top-20 left-1/2 transform -translate-x-1/2 z-[60] w-[92%] max-w-sm transition-all duration-700 ease-in-out ${isVisible ? 'translate-y-0 opacity-100' : '-translate-y-8 opacity-0 pointer-events-none'}`}>
      <div className="bg-black/90 backdrop-blur-md text-white px-4 py-3 rounded-2xl shadow-xl flex items-center space-x-3 border border-green-glow/30">
        <div className="bg-green-glow/20 p-2 rounded-full flex-shrink-0 animate-pulse">
             <Icons.CheckCircle size={16} className="text-green-glow" strokeWidth={3} />
        </div>
        <div className="flex-1">
            <p className="text-xs font-medium leading-relaxed">
              <span className="font-bold text-green-glow">{notification.name}</span> {notification.message}
            </p>
        </div>
        {/* Progress Bar */}
        {isVisible && (
          <div className="absolute bottom-0 left-0 h-1 bg-green-glow/50 rounded-b-2xl animate-progress-bar"></div>
        )}
      </div>
      <style>{`
