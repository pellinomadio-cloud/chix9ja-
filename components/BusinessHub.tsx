...
const BusinessHub: React.FC<BusinessHubProps> = ({ user, onVipWithdraw, onBack }) => {
  ...
  const handleWithdraw = () => {
    ...
    setWithdrawAmount('');
    setError('');
    // Removed window.alert as per iframe restrictions
  };
  ...
  return (
    <div className="px-4 py-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
      ...
      {/* VIP Status Banner */}
      {user.isVIP ? (
        <div className="bg-green-glow rounded-2xl p-4 text-black shadow-lg flex items-center justify-between">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="bg-green-glow/10 p-5 rounded-2xl shadow-sm border border-green-glow/20">
           <p className="text-xs font-bold text-green-glow uppercase mb-1">VIP Business Funds</p>
           <p className="text-3xl font-black text-green-glow">₦{vipBalance.toLocaleString()}</p>
           <p className="text-[10px] text-green-glow/70 mt-1 italic">Funds available for instant withdrawal</p>
        </div>
      </div>
        ...
        <div className="bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-800 space-y-4">
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="0.00"
                  className="w-full p-3 pl-8 bg-black border border-gray-800 rounded-xl text-lg font-bold text-white outline-none focus:ring-2 focus:ring-green-glow"
                />
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full py-4 bg-green-glow hover:bg-green-dark text-black font-black rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center space-x-2"
            >
      ...
      <div className="bg-green-glow/5 p-4 rounded-xl border border-green-glow/10">
         <p className="text-xs text-green-glow/70 leading-relaxed">
