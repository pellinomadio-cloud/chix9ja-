  ...
  return (
    <div className="px-4 py-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex items-center justify-center p-4 bg-green-glow/10 rounded-full text-green-glow mb-2">
        ...
        <p className="text-sm text-gray-500">Unlock the full power of chix9ja with a lifetime professional VIP status.</p>
      </div>
        ...
        <ul className="space-y-3">
            {[
                ...
                <li key={i} className="flex items-center space-x-3 text-sm text-gray-400">
                    <Icons.CheckCircle size={18} className="text-green-glow shrink-0" />
                    <span>{benefit}</span>
                </li>
            ))}
        </ul>
      ...
      <div className="space-y-3">
          <button 
            onClick={onProceed}
            className="w-full py-4 bg-green-glow hover:bg-green-dark text-black font-black rounded-xl shadow-lg transition-all transform active:scale-95 flex items-center justify-center space-x-2"
          >
